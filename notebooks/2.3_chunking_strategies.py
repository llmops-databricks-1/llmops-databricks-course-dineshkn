...
# MAGIC - Analyzing chunk statistics
# MAGIC - Alternative chunking implementations
...
# MAGIC - Extracts elements (paragraphs, code, headings, etc.)
# MAGIC - Each element becomes a chunk
# MAGIC - **This is what we used in Notebook 2.2!**
...
# MAGIC
# MAGIC We'll use the chunks extracted and cleaned in Lecture 2.2.

# COMMAND ----------

chunks_df = spark.table(f"{catalog}.{schema}.causal_inference_chunks")
...
# MAGIC Note: `element_type` is used during processing to filter out tables/figures
# MAGIC but is not stored in the chunks table — columns are `id`, `paper_id`, `chapter_num`, `title`, `summary`, `text`.
...
# MAGIC %md
# MAGIC ## 6. Chunk Size Recommendations
# MAGIC
...
# MAGIC
# MAGIC ### ✅ Do:
# MAGIC 1. **Preserve metadata** (paper_id, title, chapter_num, etc.)
# MAGIC 2. **Test different chunk sizes** for your specific use case
# MAGIC 3. **Use overlap** for better context (50-100 characters)
# MAGIC 4. **Monitor chunk quality** (length distribution, content quality)
# MAGIC 5. **Filter low-quality chunks** (too short, only whitespace/numbers)
# MAGIC
# MAGIC ### ❌ Don't:
# MAGIC 1. Split in the middle of sentences (unless using fixed-size)
# MAGIC 2. Ignore document structure (code blocks, equations, etc.)
# MAGIC 3. Lose metadata during chunking
# MAGIC 4. Use the same chunk size for all document types
